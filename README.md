...
`status` is one of `planning`, `awaiting_approval`, `revising`, `finalizing`, `finalized`, `failed`. A decision takes as long as the original proposal did, so the job reports `revising` or `finalizing` while that runs rather than still awaiting review.

Errors: `415` on a non-`.docx`, `413` over 25 MB, `400` on an empty file, `404` on an unknown job, `409` when deciding on a job that isn't awaiting review or already has a decision in progress, or downloading one that isn't finished, `410` if the output file has been deleted, `503` if the editor process is unavailable — the checkpoint is untouched, so the same call can be retried.
...
```bash
uv run pytest                 # 45 tests, no API key, no subprocess, ~seconds
